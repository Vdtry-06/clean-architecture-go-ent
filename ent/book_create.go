...
func (bc *BookCreate) SetPublishYear(i int) *BookCreate {
	bc.mutation.SetPublishYear(i)
	return bc
}
...
func (bc *BookCreate) check() error {
	...
	if _, ok := bc.mutation.PublishYear(); !ok {
		return &ValidationError{Name: "publish_year", err: errors.New(`ent: missing required field "Book.publish_year"`)}
	}
	if v, ok := bc.mutation.PublishYear(); ok {
...
func (bc *BookCreate) createSpec() (*Book, *sqlgraph.CreateSpec) {
	...
	if value, ok := bc.mutation.PublishYear(); ok {
		_spec.SetField(book.FieldPublishYear, field.TypeInt, value)
		_node.PublishYear = value
	}
	return _node, _spec
}
